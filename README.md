...
**絡む価値 / バズる / 炎上 / スルー / 誤解 / AI臭** の 6 つの校正済み確率を本文の下に表示する Chrome 拡張。
...
- `background.js`：`POST https://api.typesafe.ai/v1/systemone` に `state`（投稿）と 6 つの Noul 質問（`questions.js`）を 1 リクエストで送信。429/529 は指数バックオフ。1 分あたりの上限はオプションで設定。
